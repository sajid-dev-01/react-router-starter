...
export function KBar({ children }: { children: React.ReactNode }) {
  ...
  const actions = useMemo(
    () =>
      navData.navMain.flatMap((navItem) => {
          ...
          <KBarAnimator className="bg-background text-foreground relative !mt-64 w-full max-w-[600px] !-translate-y-12 overflow-hidden rounded-lg border shadow-lg">
            <div className="bg-background">
              <div className="border-x-0 border-b-2">
                <KBarSearch className="bg-background w-full border-none px-6 py-4 text-lg outline-none focus:outline-none focus:ring-0 focus:ring-offset-0" />
              </div>
              <RenderResults />
            </div>
          </KBarAnimator>
